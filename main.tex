...
\textbf{Data source mapping}: Case incidence data is typically readily available with very frequent (often daily) updates and high timeliness. However, it is an imperfect and potentially biased proxy for infection incidence, and is dependent on testing policies and lab processing and reporting timeliness. 
...
\textbf{Workflow Demonstration:}
\begin{enumerate}
    \item \textbf{Process DAG iteration:} 
    Suppose that the concentration $W_t$ of viral RNA in wastewater on day $t$ depends on the time series of infections up to day $t$ and the average amount of viral RNA that an individual sheds into wastewater $s$ days after being infected. This can be represented in the process DAG (Figure X) and by the following equation for $W_t$:
    \begin{equation} \label{eq:wastewater}
        W_t \sim \Gamma\left( \frac{\alpha}{N}\sum_{s=1}^\infty I_{t-s}u_s, b   \right)
    \end{equation}
    where $\alpha>0$ is a constant representing the average total amount of RNA that an infected individual sheds over the course of their infection, $N$ is the population size, $u_s$ is the average shedding rate $s$ days after infection, normalised such that $\sum_{s=1}^\infty u_s=1$, and $\Gamma(\mu,a)$ denotes a gamma distribution with mean $\mu$ and shape $a$. This was similar to the wastewater model used by REF; other choices of continuous non-negative-valued distributions are possible for $W_t$ such as a log-normal distribution.
    ...
    \item \textbf{Observation DAG:} 
   Wastewater samples are typically collected at some cadence from one or more sampling sites, and the concentration of genetic material from the pathogen quantified via PCR testing. 
   The existence of multiple sites and non-contemporaneous sampling frequencies complicates the interpretation of quantitative wastewater data, which can be modelled at varying levels of complexity.  
    ...
    More complex models could incorporate other factors such as individual-level and site-level variation, catchment population dynamics, and environmental degradation.
    \item \textbf{Integration choice:} As in case study 1, we recommend that a modular approach with sequential consistency assessment is initially taken to detect and resolve data source conflicts. Independent estimates of $R_t$ from these models could then be combined by ensembling, or a joint model could be fitted with a shared $R_t$ state. 
