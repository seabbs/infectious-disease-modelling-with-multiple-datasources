...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/schematic workflow_cirm output.drawio.png}
    \caption{Recommended workflow for integrating multiple data sources in infectious disease modelling. (Final version reported by the end of the workshop)}
    \label{fig:workflow}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Recommended workflow for integrating multiple data sources in infectious disease modelling.}
    \label{fig:workflow}
\end{figure}
...
Each case study follows our complete workflow framework, demonstrating how different data sources enable analyses to answer questions that would require strong assumptions with other sources alone.
...
We refer to these as $O_{000}$ (for the simplest observation model not modelling any of the above) to $O_{111}$ (for the most comprehensive accounting for all three observation features).
...
This is obtained by choosing one process model, one observation model and one fitting and integration method (the latter is of course dependant on the choice of process and obseravation model and their properties but also guided by logistical constraints etc). 

For this case study, combinations of the 3 process models and 8 ($2\times 2 \times 2)$ observation models give 24 possible models, on top of which fitting methods can be chosen for each. We can give 2 or 3 examples out of those from the literature:

\begin{itemize}
    \item EpiEstim – Poisson and none of the observation model assumptions – hidden assumptions are: no underreporting, no delays in reporting, no noise. So that is $P_2-O_{000}$. This is fitted using analytic posterior (conjugate prior)
    \item EpiNow2 (may need to specify a version) – Deterministic with no underreporting but with delays and random noise so $P_1-O_{011}$. This is fitted using XXX
...
E.g. EpiEstim’s main strength is computational speed, weakness is unrealistic assumptions about no delays, $100\%$ reporting etc. Discuss which of these matters most, depends on context perhaps, and balance between need for speed and realism of these assumptions
Common weakness of all these is they assume GT is constant and perfectly known. Mention that some versions e.g. of EpiEstim extend the approach to relax this assumption, and refer to case study 4 which relaxes this too.
