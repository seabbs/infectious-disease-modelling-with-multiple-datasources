...
We recommend following a structured, iterative workflow for multi-data source modelling (Figure~\ref{fig:workflow}) that intends to make modelling choices transparent, assumptions explicit, and provides principled tools for assessing model adequacy (regardless of whether final inference is Bayesian or frequentist). Our workflow extends established Bayesian workflow principles [@placeholder]—with their emphasis on iterative model building, prior predictive checks, computational diagnostics, and posterior predictive assessment—to the specific challenges of multi-source infectious disease modelling.
...
For each data source, develop an \textbf{observation DAG} linking the underlying process to observed data through measurement models and reporting mechanisms. If needed, revise your \textbf{process DAG} assumptions in light of the data sources, e.g if you can collapse processes from individual- to population-level.

Once you have developed your process and observation DAGs, proceed to \textbf{model specification and validation}, including prior specification, parameter identifiability assessment, and diagnostic approaches. These considerations may also lead you to revise your process and observation DAGs. 
You then face the key decisions: \textbf{``What to combine?''} and \textbf{``How to combine?''}. At this stage, practical constraints (e.g., different teams using incompatible programming languages, conflicting data sources, or unamanageable model complexity) may necessitate revisiting the data mapping, process and observation DAGs steps.
