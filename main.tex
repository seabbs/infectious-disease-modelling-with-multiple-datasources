...
Infectious disease models can inform critical public health decisions for both epidemic and endemic diseases, with applications including estimation of key epidemiological parameters, interpretation of surveillance data, and quantifying the expected impact of interventions and policies~\citep{heesterbeek15_modeling,morgan19_how,becker21_developmentcovid,bershteyn22_realtimeinfectious,leask25_informing}.
In spite of this relevance for guiding decision making and policy, infectious disease models are often developed without a consistent process.
Assumptions and data sources are often insufficiently documented~\citep{jalali20_transparencycovid,pokutnaya23_interidmrccovid}, as are calibration and validation practices~\citep{dankwa25_calibration}.
The resulting variability in quality and transparency makes it difficult to assess, reproduce or independently verify analyses and their suitability for addressing a given question of relevance.

% Paragraph 2: Current Approaches and particular challenges
Structured frameworks such as the Bayesian workflow can address these challenges through iterative development, systematic criticism, and principled uncertainty quantification~\citep{green2003highly,gelman2020bayesian, nicholson2022interoperability}.
However, infectious disease models face particular challenges for implementing such workflows.
In outbreak settings, new data streams emerge over time, surveillance systems evolve to meet changing needs, and models must be developed under time constraints with usually limited understanding of novel data sources \citep{metcalf17_opportunities,morgan19_how,mccaw2023role}.
These issues are not unique to infectious disease modelling, but they are amplified by the complexity of integrating diverse types of data.
Even outside emergency response, models frequently draw on multiple, imperfect data sources posing particular challenges including: detecting and resolving conflicts between data sources, e.g., when different data streams suggest different dynamics; combining data sources with different spatial or temporal resolutions; and navigating branching decision paths where multi-data source integration choices impose model structure constraints \citep{deangelis2018analysing, nicholson2022interoperability}.
Joint modelling approaches can, in principle, address some of these challenges by improving uncertainty quantification and parameter identifiability, but often at the cost of increased computational complexity and model specification effort~\citep{russell2024combined,lison2024generative}.
...
Although some work has systematically compared modelling choices for specific case studies \citep{bouman2024bayesian} or provided workflow guidance focused on specific computational frameworks \citep{grinsztajn2021bayesian}, structured guidance remains limited \citep{Behrend2020-au, nicholson2022interoperability, deangelis2018analysing}.
...
The Bayesian paradigm, combining prior information with data, is a natural framework for infectious disease modelling, since such models require at a minimum prior assumptions, usually both prior information and at least one data stream, and potentially, multiple data streams. Our approach, therefore, builds on established Bayesian workflows \citep{green2003highly,gelman2020bayesian}, which in turn build on Box's scientific method \citep{box1979robustness,Box1980}, extending them for the specific challenges of infectious disease modelling, including evolving research questions, emerging data sources, and time-constrained development.
...
Our workflow includes defining the research question, developing \ac{DAG} representations of disease and observation processes in a state-space modelling framework, model modularisation, inference and computation choices, model specification and validation, integration method selection, and implementation and reporting considerations.
