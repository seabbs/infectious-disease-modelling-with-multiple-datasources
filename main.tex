...
\author[7]{@invited-survey-respondees}
\author[8]{Mircea T. Sofonea}
\author[9, 10]{Pierre Nouvellet}
\author[11]{Michael Plank$^*$}
\author[10]{Anne Cori$^*$}
\author[12]{Anne Presanis$^*$}
...
Starting with simple sub-models makes it easier to diagnose issues such as poor fit, model misspecification or convergence issues. Complexity can then be added incrementally, only as far as needed. Sequentially combining sub-models also facilitates the detection of inconsistencies or conflicts between them \citep{presanis2013conflict,manderson2023combining}. Modular approaches also offer computational advantages over joint modelling. 
...
Prior predictive checks simulate data from the prior to ensure model behaviour aligns with epidemiological plausibility before seeing data; and to assess prior-data conflict \citep{yang2025detecting}.
...
Detecting and resolving conflicting evidence about underlying latent processes is essential to ensure coherent inference and robustly informed decisions \citep{presanis2013conflict,sherratt2021exploring}. Such conflicts may arise between prior assumptions and data (``prior-data conflict''), between different partitions of a joint model into submodels, or even within a data source but between different units of data \citep{presanis2013conflict,yang2025detecting}.
...
When employing ensembles, careful attention to ensuring all models target the same estimand, resolving temporal alignment, and documenting methodological assumptions becomes essential for meaningful combination. The following sections have additional detail.

In addition to prior-data conflict and submodel conflict, different data sources may also conflict. This should be assessed by repeating the model validation and specification process as discussed in \ref{sec:spec-validate} \citep{presanis2013conflict,sherratt2021exploring} once the data integation process has been decided up.
...
Sequential Monte Carlo (SMC) methods, or particle filters, approximate the posterior using weighted particles and enable online state estimation with strong real-time capabilities \citep{doucet2001introduction},  making them particularly valuable for real-time epidemic tracking
where new data arrive continuously \citep{birrell2020efficient, storvik2023sequential}.
...
Complex multi-source models are fundamentally software development projects requiring professional engineering practices \citep{gelman2020bayesian}.
We recommend standard software development practices: unit testing to verify model components, continuous integration to catch errors early, and comprehensive test coverage for data processing and diagnostics. These practices become essential as models grow in complexity, preventing errors that could undermine public health decisions.
...
We refer back to the data sources and workflow frameworks introduced in Sections XXX and YYY. This section presents a case study illustrating the application of our proposed evaluation framework. 

As the authors comprise individuals from diverse disciplinary backgrounds with experience working across a wide range of data types, we designed a questionnaire informed by a broad spectrum of perspectives. Building on the checklist of data characteristics proposed to assess the strengths and weaknesses of data streams, we developed targeted questions aligned with each characteristic to facilitate systematic and accessible characterisation. These questions (see Supplementary Material XXX) were designed to support both qualitative and quantitative evaluation across data sources. Following the development of this questionnaire, both the authors and[??] completed it for a range of data sources.

Next, we identified a focused set of data sources and their core characteristics, which are considered in the subsequent case studies. These sources include time series of confirmed cases, wastewater surveillance data, deaths[mortality records?], and transmission-paired observations. The relevant categorical variables for each data stream, derived from questionnaire responses targeting each core characteristic, are summarised in Table \ref{tab:survey_results}. Quantitative assessments, collected using a 1-5 scale, are illustrated in Figure \ref{survey_responses}.  

These survey responses revealed substantial variation across participants when answering the same questions for a given data source, reflecting the influence of differences in subjective interpretation, context, and experience. For example, while confirmed time-series were uniformly described as routinely collected and targeted for the whole population, responses diverged on aspects such as stratification, reporting frequency, and the epidemiological quantities informed (Table \ref{tab:survey_results} and Supplementary Material XX). Such discrepancies result in differing characterisation of the same data stream, leading to divergent assessment of its overall core characteristics. This variation highlights the importance of acknowledging context-specific practices and perceptions when evaluating data sources, Nevertheless, the six core characteristics--- metadata, scope, resolution, data quality, data utility, and practical considerations---provide a systematic framework that can be used as a systematic checklist to guide structured evaluations of the data sources, enabling context-sensitive assessments of their strengths and limitations. 
