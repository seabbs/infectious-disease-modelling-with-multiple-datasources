...
Inference for infectious disease models typically involves estimating a set of parameters $\theta$ from observed data $Y$, which may include case counts, hospitalisations, deaths, wastewater surveillance or other epidemiological measurements. These data are assumed to arise from a probabilistic model characterised by a likelihood function $p(Y | \theta)$ that links the parameters to the data through both an underlying transmission dynamics (represented by the process DAG (Section \ref{sec:process}) in our workflow) and a noisy observation mechanism (captured by the observation DAG (Section \ref{sec:observation})). However, choosing an appropriate inferential approach becomes increasingly complex when integrating multiple data sources, as researchers must balance model complexity, computational feasibility, likelihood tractability, and available expertise, decisions that are often interdependent and may require iterative refinement.

The fitting choices workflow (Figure \ref{fig:fitting}) illustrates the systematic decision-making process required when selecting appropriate inference methods for integrating multiple data sources in infectious disease modelling. The workflow contains four interconnected components. Model Complexity Assessment evaluates key factors such as dimensionality, hierarchical structure, likelihood tractability, and computational constraints to determine the feasibility of different fitting approaches. This assessment directly informs the critical choice between two primary methodological pathways: likelihood-based methods (when the likelihood function $ p(Y | \theta)$ is analytically or computationally tractable) and likelihood-free methods (when the likelihood is intractable or unavailable), each requiring detailed consideration of their advantages, limitations, and implementation challenges. Throughout the fitting process, Diagnostics procedures provide essential feedback on model performance and computational efficiency: convergence checks verify that parameter estimates have stabilized across chains or iterations, sensitivity analysis assesses robustness to prior specifications and different model assumptions, and evaluation of computational trade-offs balance between inferential accuracy and feasibility constraints. Implementation Considerations address the real-world challenges, including making decisions around software selection and managing compatibility requirements across different computational environments. The workflow's iterative nature is particularly important in practice, as computational limitations or fitting difficulties may require researchers to revisit earlier decisions about model structure (returning to Process DAG development) or integration strategy (reconsidering Data Integration Choices).

The following subsections provide detailed guidance on navigating these methodological choices in practice. We begin with likelihood-based methods (Section \ref{sec:like-base}), then turn to likelihood-free approaches (Section \ref{sec:like-free}), discussing when these methods become necessary and how to implement them effectively. Since practical implementation often determines modelling strategies, we first address implementation considerations (Section \ref{sec:implement}), exploring concrete challenges such as software environment compatibility, computing resource constraints. Based on this consideration, we discuss the selection of the appropriate inference approach (Section \ref{sec:select-tool}), highlighting how different computational frameworks align with model complexity, data integration demands, and methodological choices.
...
\subsubsection{Likelihood-based methods} \label{sec:like-base}
...
\subsubsection{Likelihood-Free Methods}  \label{sec:like-free}
...
\subsubsection{Implementation Considerations}  \label{sec:implement}

A wide range of software tools is available to support the implementation of both likelihood-based and likelihood-free methods in infectious disease modelling. When selecting software for infectious disease modelling, the primary consideration should be matching the tool's strengths to the specific modelling requirements and computational constraints. 

Bayesian inference platforms such as Stan, JAGS, NIMBLE, PyMC, Turing.jl, and NumPyro are widely used for likelihood-based approaches \citep{carpenter2017stan,abril2023pymc, ge2018turing,fjelde2025turing, phan2019composable}. Stan is good for high-dimensional problems through efficient HMC algorithms, while JAGS and NIMBLE offer flexibility for model structures that may not well-suited to gradient-based methods. For models requiring rapid inference with large datasets, INLA and VI are good alternatives. INLA implementations are available in R-INLA package \citep{martins2013bayesian}, Stan, and TMB (Template Model Builder) \citep{osgood2023statistical}, while VI methods are supported across multiple platforms including PyMC, Turing.jl, Stan, and NumPyro. Additionally, tools such as LibBi support SMC-based inference for state-space models and can scale to high-dimensional problems using GPU and parallel computing \citep{murray2015bayesian}. 

For likelihood-free methods, the decision typically centers on computational scalability. Simple ABC and BSL implementations are sufficient for low-to-moderate-dimensional problems and are supported in packages/ softwares like EasyABC, abc, abctools, and ELFI\citep{jabot2013easy,csillery2012abc,nunes2015abctools, lintusaari2018elfi}. However, more complex scenarios may require specialized tools that support parallel and GPU computing. 

% Add some stuff talking about performance on CPU vs GPU etc
Computational scalability is essential for large-scale epidemiological studies, where the choice of inference method should align with available high-performance computing (HPC) resources and parallelization opportunities. The parallelization potential varies across inference methods, when working with extensive datasets or complex models, one should consider the following computational trade-offs. MLE is highly parallelizable across data points, making it efficient scaling on both multi-core CPUs and GPUs for large datasets. Traditional MCMC methods have limited parallelization due to their sequential chain structure, though within-chain parallelization is possible for computationally expensive likelihood evaluations, and while GPU implementations are available in many settings, they are generally most effective for operations involving large matrix multiplications rather than the sequential parameter updates. Particle-based methods (PMCMC, SMC) benefit from particle-level parallelism and are well-suited for many-core GPU architectures, where thousands of particles can be processed simultaneously \citep{henriksen2012parallel}. However, INLA has low parallelization potential due to its dependence on sequential matrix operations that resist decomposition. VI scales well when processing small chunks of data at a time, where gradient calculations for each chunk can run in parallel \citep{hoffman2013stochastic}. Most modern VI implementations commonly use GPU acceleration, making them quite efficient for large datasets \citep{bingham2019pyro}. Likelihood-free methods like ABC excel in parallelization, as simulations are trivially parallelizable and GPU-friendly, though practitioners should account for potentially high memory requirements when scaling to many cores \citep{kulkarni2022hardware}. BSL allows parallelization with GPU acceleration to be possible for computationally intensive density estimation steps. When selecting methods for HPC environments, practitioners should carefully weight these parallelization capabilities against memory constraints, available hardware, and the specific computational demands of the epidemiological applications.

\subsubsection{Selecting the Right Inference Approach}  \label{sec:select-tool}

Selecting an appropriate inference method in infectious disease modeling requires carefully balancing between model complexity, computational constraints, statistical accuracy and data availability \citep{funk2020choices}. Rather than adopting a universal approach, the choice should be tailored to the specific modeling context and inferential goals. In practice, this decision is best approached as part of an iterative process, where model structure and inferential approach are continuously refined as new sources of information emerge and outbreak conditions change. Table \ref{tab:methods_comparison} provides a concise, though not exhaustive, reference to help assess these trade-offs and to guide selection based on typical use cases. 

For general scenarios with computationally tractable likelihoods, Bayesian MCMC methods are the most interpretable and reliable solution and provide a good description of the uncertainty with high statistical rigor. In most practical applications, we recommend using NUTS as the default MCMC algorithm whenever possible, as it efficiently explores high-dimensional parameter spaces with minimal tuning. When there are multiple heterogeneous data sources to be merged, particle-based approaches, such as PMCMC, should be particularly considered. Its strength lies in efficiently handling complex, stochastic latent dynamics and combining diverse data streams through sequential updating within a single probabilistic framework. The trade-off is higher computational demand, requiring careful consideration of hardware resources and parallelization strategies.


In settings where computational speed, repeated analyses, or large-scale scenario exploration are priorities, approximate methods such as VI or ABC-SMC might offer scalable alternatives \citep{chatzilena2019contemporary, engebretsen2023real}. VI is highly efficient for high-dimensional, differentiable models and typically much faster than MCMC, although it trades some accuracy for speed and may not always capture full posterior uncertainty. ABC-SMC is flexible when likelihoods are unavailable but simulations are feasible, though it can be computationally expensive due to repeated model simulation. Both approaches balance inferential precision against speed and scalability, making them particularly useful for iterative scenario exploration and timely decision-making.


Within a workflow-guided strategy, we recommend starting with the simplest feasible full-likelihood Bayesian approach, using NUTS, and escalating to particle-based approaches when integration of multiple data streams, sequential updating or model complexity exceeds the capabilities of MCMC.  Approximate methods can be reserved for scenarios requiring rapid iteration or large-scale simulations, where some loss of precision is an acceptable trade-off for scalability. Regardless of the chosen approach, iterative model checking, sensitivity analysis, and validation across data streams remain essential to produce credible and actionable insights, particularly in evolving outbreak contexts.


% Beyond computational efficiency, statistical accuracy is influenced by algorithmic choices including priors distributions, tuning parameters, tolerance thresholds, and the number of particles or iterations. Full-likelihoods methods typically yield better estimator performance, subject to likelihood tractability and proper specification, whereas likelihood-free approaches often prioritise flexibility and scalability over precision \citep{alahmadi2020comparison}. 
