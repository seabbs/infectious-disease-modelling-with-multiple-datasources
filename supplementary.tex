...
% \paragraph{Combining estimates via weighted ensembling:}
% Separate inference produces two time series of $R_t$ estimates with associated uncertainties. As noted in the main text, any inconsistencies between these estimates may indicate the need to refine the underlying models. Once this refinement is complete, the estimates are synthesized into a consensus time series using a weighted ensemble function, i.e., $R_{1:T} = \mathcal{F}_{\mathrm{ens}}\big(R_{1:T}^\mathrm{cases}, R_{1:T}^\mathrm{deaths}, \sigma_{1:T}^\mathrm{cases}, \sigma_{1:T}^\mathrm{deaths}\big)$.
